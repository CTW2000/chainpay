    ...
    public SendResult tick() {
        try {
            ...
            if (r.halted()) {
                ...
                log.warn("发送这一轮提前结束，下一轮再来：{}（本轮签 {}、广播 {}、重发 {}、加价 {}）", r.detail(), r.signed(), r.broadcast(), r.resent(), r.bumped());
            } else if (r.examined() > 0 || r.resent() > 0 || r.bumped() > 0) {
                log.info("发送：看了 {} 笔，签 {}、广播 {}、重发 {}、加价 {}、判失败 {}", r.examined(), r.signed(), r.broadcast(), r.resent(), r.bumped(), r.failed());
            }
            return r;
        } catch (RuntimeException e) {
            log.error("发送任务异常，这一轮中止，下一轮重试：{}", e.toString());
            return SendResult.retryLater(0, 0, 0, 0, 0, 0, e.toString());
        }
    }
}
