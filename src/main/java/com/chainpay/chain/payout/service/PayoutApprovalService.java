...
/**
 * 管理侧：待核准列表、核准、拒绝、限额。改的是 payout 的状态与限额表，只有系统身份有 UPDATE，所以整段用系统身份（限定名 system 的 JdbcClient 与账本）。
 * 拒绝 = 解冻 + REJECTED + 原因，几条写一起成败，一个系统事务；列表、核准、限额各是一条语句，直接走系统连接。
 * 核准只是 PENDING_APPROVAL → QUEUED，之后和普通申请一样由发送任务处理。
 */
@Service
public class PayoutApprovalService {
    ...
    public PayoutApprovalService(@Qualifier(SystemLedger.QUALIFIER) JdbcClient systemJdbc, @Qualifier(SystemLedger.QUALIFIER) LedgerService systemLedger) {
