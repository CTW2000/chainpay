...
/**
 * 注资登记（M6-②）：运营往热钱包充的币，账本不记（不是任何商户的钱），但 M5 的托管等式要能解释它，否则每一轮对账都报「链上多」。
 * 运营只指认「是哪一笔」（交易哈希，同一笔里多条时再给 logIndex）；金额、块、代币全部从索引器已经记下的日志里读——数字不由人填。
 * 四道门：日志得在库里（404）；收款方得是热钱包（否则那是入账，也是 404「没有转给热钱包的日志」）；发起方不能是平台自己的地址（收款地址或热钱包，那是归集，400）；
 * 块得 ≤ finalized（409）——会被重组翻掉的钱不进等式。同一条日志登记两次返回同一行。
...
public class HotWalletFundingService {
    ...
    public HotWalletFundingService(SystemLedger system) {
        this.system = system;
    }
    ...
    public List<Funding> list() {
        return system.inTransaction(s -> s.jdbc().sql(SELECT + " ORDER BY f.id DESC LIMIT 200").query(this::map).list());
    }
