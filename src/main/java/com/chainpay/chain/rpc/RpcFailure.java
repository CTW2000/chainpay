...
/**
 * 一次节点失败该怎么处理（2026-09-16）。
 *
 * <p>{@link JsonRpcException#code()} 说的是失败<b>从哪一层来</b>（正文里有没有 error 对象），
 * 调用方要的却是<b>该做什么</b>。两者不是一一对应的，这一点已经被现实打脸三次，修法都写在注释里：
 * Alchemy 的 429 带 code 429、401 带 code -32000（见 {@link JsonRpcClient}），
 * 链头附近单块取日志失败也带码（见 BlockIndexer）——每次都是把一个带码的错误重新翻译回「没问到」。
 *
