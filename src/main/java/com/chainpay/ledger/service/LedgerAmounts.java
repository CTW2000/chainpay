...
/**
 * 金额的规矩只在这里写一份（2026-09-21）：<b>账本装得下多大的数，以及这个数对外怎么写</b>。
 *
 * <p>这是同一个概念的两面，所以放在一起：写法里的「20 位整数、18 位小数」就是容量本身，改了一个另一个必须跟着改。
 * 当天先拆成了两个类（容量在这里，写法在 common/web），用户看出两者语义相同、再这样拆下去收口的类会散落各处，合回一个。
 * 规矩：<b>收口按概念，不按层</b>——一个概念一个家，放在定义这个事实的地方（金额列的类型是账本定的）；
 * 换算是另一个概念（链上原始单位按 decimals 换成账本金额），留在 {@code TokenAmounts}，它依赖这里，反过来不行。
 *
 ...
 * （{@code LedgerServiceImpl.validate}、{@code TokenAmounts.toLedger}）。{@code SchemaGuardTest} 把这两个数和库里每一个
 * 带小数位的列绑在一起：列改了或这里改了都红。此前「18」散在四处，把账本那份收紧成 17，全套测试一条都不红。
 *
 ...
 * 逐位写全是 10 亿个字符，实测一个请求打挂进程）、一百万位的普通写法（实测光解析就 11 秒）、{@code +1}、{@code 1.}、
 * 别的文字的数字（阿拉伯-印度数字它也认；Java 正则的 {@code \d} 只认 ASCII 的 0–9）。
 * 两个正则给请求记录的 {@code @Pattern} 用，{@code ControllerBoundaryTest} 守着：控制器里每个
...
public final class LedgerAmounts {
     ...
     * 只管写法、不管容量：超出容量但没超过 64 位的，由账本拒——错误码 2004、消息说清几位，
     * 这是转账接口自 M1.5 起的契约（{@code ApiContractTest} 钉着）。
     */
     ...
     * 提现与限额接口自 M4-④ 起一直这样：超出容量在边界就回 2001。
     */
     ...
     * int 相减 1 − (−2147483647) 溢出成 −2147483648，「超过 20 位」的检查就被一个负数绕过去了（2026-09-21 实测）。
     */
    ...
    /**
     * 装得下就原样返回；装不下就用调用方给的异常报出原因。全项目「装不装得下」只在这里判（2026-09-22 收口）：
     ...
     * （2026-09-21 实测一个请求打挂进程）。这里只读 scale 与 precision，代价与数有多大无关。
     */
     ...
     * null 仍是 null。全项目金额变成字符串只经这里（2026-09-22 收口，{@code SingleHomeGuardTest} 守着）——此前散在七处，
     * 其中 {@code String.valueOf} 那处把 1 wei 写成 {@code 1E-18}，和别处的写法不一样。
     *
