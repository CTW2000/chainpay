...
/**
 * M6-⓪ · 把四个指示器装进健康端点，并给系统池绑指标。
 * bean 名去掉 HealthIndicator 后缀就是部件名（indexer / hotWallet / audit / systemDb），application.yml 的分组按这些名字引用。
...
class OpsHealthConfig {
    ...
    @Bean
    SystemDbHealthIndicator systemDbHealthIndicator(SystemLedger system) {
        return new SystemDbHealthIndicator(system::ping);
    }

    /** 主池由 Boot 自动绑（它是 DataSource bean）；系统池不是 bean，手工绑到同一个注册表，同名指标靠 pool 标签分开。 */
    @Bean
    MeterBinder systemPoolMetrics(SystemLedger system) {
        return system::bindMetrics;
    }
}
