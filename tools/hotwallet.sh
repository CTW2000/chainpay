#!/usr/bin/env bash
# 离线热钱包工具：从助记词算出 m/44'/60'/1'/0/0 的私钥与地址（私钥只显示一次）。
#
#   1. 先断网。
#   2. 先编译一次（需要联网下载依赖的话在断网前做）：JAVA_HOME=~/.local/jdk-25/Contents/Home mvn -q compile
#   3. tools/hotwallet.sh   然后按提示输入助记词（不回显）；粘好私钥后清屏
#
...
exec "$JAVA_HOME/bin/java" -cp "target/classes:$(cat "$CP_FILE")" com.chainpay.chain.wallet.HotWalletTool
