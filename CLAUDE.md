...
└── chain/               只读上链（M2）
    ...
    ├── payout/          付款（M4）：domain 状态机（PayoutStatus / PayoutTxStatus 显式转换表）、service 三笔账本流（PayoutLedger）与发送任务（PayoutSender / FeePolicy）、repository 热钱包行与发送用的提现与尝试、config 装配（设了私钥才装签名器；私钥 + 主节点都有才装发送任务）
