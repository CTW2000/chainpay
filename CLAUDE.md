...
- **原语与协议编码用库，策略与业务自己写**（2026-09-09 由用户改判）：密码学原语（Keccak、曲线、ECDSA、HMAC、PBKDF2）一直是 BouncyCastle；协议层编码（RLP、EIP-1559 布局、签名打包与恢复、地址）改为 web3j 6.0.0 的 `crypto` 模块（不拿 `core`；排除 EIP-4844 的原生库 jc-kzg-4844），我们只留不依赖 web3j 类型的薄包装（`Rlp.Item`、`Eip1559Transaction`、`Ecdsa`、`HotWalletSigner`），换库只动这一层。**测试是我们的，实现是库的**：官方向量与自洽测试原样保留，成为对库的验收，升级版本由它们把关。JSON-RPC 客户端仍自写（正文封顶、超时、错误分类、双节点核对是策略，不是轮子）。M3 的 BIP-32/39 与 xpub 解析不动：web3j 不覆盖 xpub 字符串，唯一能覆盖的 bitcoinj 太重，那三百行已过官方向量
