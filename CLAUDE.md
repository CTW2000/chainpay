...
- **热钱包私钥**（M4-①，2026-09-09）：服务端唯一的一把私钥，从环境变量 `CHAINPAY_PAYOUT_HOT_WALLET_KEY` 装入一次，进程里只活在 `HotWalletSigner` 里（只暴露 `address()` 与 `sign(tx)`，`toString` 只含地址；错误消息只说长度与范围，永不回显内容）。它**必须与收款树隔离**：另一句助记词，或同一句的硬化账户 `m/44'/60'/1'/0/0`（`tools/hotwallet.sh` 断网算，私钥只打印一次）——普通派生的子私钥减去由 xpub 就能算出的偏移量就是父私钥，硬化派生的偏移量用父私钥算、拿 xpub 算不出。`WalletBoundaryTest` 的禁用符号扩到 `Ecdsa` / `HotWalletDerivation` / `HotWalletTool`：payout 包只能拿签名器，拿不到裸私钥。`tools/check-secrets.sh` 扫仓库（git 跟踪 + 未忽略的文件）与任意目录：带 key/private/secret/mnemonic/xprv 字样的行里的 64 位十六进制、xprv 长串、整行 12/24 个小写单词；公开测试密钥在 `tools/check-secrets.allow` 里**逐值**放行并注明来源，规范向量文件按路径放行；命中只打印前 6 位。`SecretScanTest` 证明它在仓库上过、且抓得住埋进去的私钥与助记词
