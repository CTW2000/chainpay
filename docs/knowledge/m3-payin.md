...
3. Alchemy 免费层把 `eth_getLogs` 限在 10 块以内（HTTP 400，code -32600「Under the Free tier plan…」）。「对半分、成功后翻倍」在固定上限上震荡：12 败、6 成、翻倍到 12 再败，
   ...
   稳态不受影响，Sepolia 每分钟才 5 块。**2026-09-09 补丁**：窗口记住失败过的尺寸、向它二分逼近（TCP 拥塞控制的形状：减半、慢加、偶尔试探），收敛在上限上后每批只问一次，连续成功 100 批才忘掉天花板试探一次；「落后很远时不受每轮 10 批限制」推到 M6（停机恢复那一族）。

**另一个发现**：入账任务与索引器共用 Spring 默认的单线程调度器。入账事务卡在锁上时 `chain_head.observed_at` 也停了——一个任务被数据库锁或慢节点拖住，另一个跟着停。
**2026-09-09 补丁**：`spring.task.scheduling.pool.size: 4`（冒烟测试用一个卡住的任务证明别的照跑，并断言线程数 ≥ 注册的任务数）；系统连接加 `lock_timeout` 5 秒。
补丁里最值钱的一条红灯：PostgreSQL 等锁超时抛 SQLSTATE 55P03，Spring 7 的 SQLSTATE 翻译器不认识 55 这一类、给 `UncategorizedSQLException`，
入账任务把它当结构性错误记成 HELD_ERROR——加了超时反而让每次等锁变成一张工单。于是系统池的 JdbcTemplate 装了翻译器，55P03 翻成 `CannotAcquireLockException`（瞬时），下一轮再来。
